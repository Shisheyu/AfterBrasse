...
--~ 		end
		...
		if not (g_vars.dionea_L1exists or g_vars.dionea_L2exists or g_vars.dionea_L3exists) and player:HasCollectible(Items.DioneaFamIdL1_i) then -- Dioanea Muscipula
			local e = Isaac.Spawn(3, Familiars.DioneaFamVariantL1, 0, player.Position, Vector(0, 0), player)
			e:AddEntityFlags(1<<21) -- FLAG_DONT_OVERWRITE
			g_vars.FAM_DioneaExists = true
		end
    end
end
