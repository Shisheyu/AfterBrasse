...
function _Stillbirth:blessingDoubtful()
		...
		if CollectibleCount == 2 then
			for i=1, #entities do
				...
				if e.Type == 5 and e.Variant == 100 and math.abs((player.Position-e.Position):Length()) <= 28 then
					...
					player:FullCharge()
					for j=i, #entities do
						if entities[j].Type == 5 and entities[j].Variant == 100 then
							entities[j]:Remove()
						end
					end
					e:Remove()
	...
	if not collectibleHasSpawned and room:IsFirstVisit() then
		if roomType == RoomType.ROOM_BOSS and isRoomOver(room) then
			local rand = math.random(#bossPool)
			local pos = room:GetRoomCenter()
			for i=1, #entities do
				if entities[i].Type == 5 and entities[i].Variant == 100 then
					pos = entities[i].Position+Vector(0,160)
					entities[i].Position = entities[i].Position-Vector(0,160)
				end
			end
			Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, bossPool[rand], pos, Vector(0, 0), player)
			collectibleHasSpawned = true
		elseif roomType == RoomType.ROOM_TREASURE then
			local rand = math.random(#itemPool)
			local pos = room:GetCenterPos() - Vector(0,160)
			for i=1, #entities do
				if entities[i].Type == 5 and entities[i].Variant == 100 then
					entities[i].Position = pos+2*Vector(0,160)
				end
			end
			Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, itemPool[rand], pos, Vector(0, 0), player)
			collectibleHasSpawned = true
		end
	end
end
